...
int builtin_cmd(char **argv)
{
	...
	else if(!strcmp(argv[0],"bg") || (!strcmp(argv[0],"fg")))     //执行相应任务
	{
		do_bgfg(argv);
		return 1;
	}
...
void waitfg(pid_t pid)
{
	while(fgpid(jobs) == pid)   //等待前台不是pid
	{
		sleep(1);
	}
	return;
...
void sigchld_handler(int sig)
{
	...
	while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)    //回收僵尸进程
	{
...
void sigtstp_handler(int sig)
{
	...
	getjobpid(jobs, pid)->state = ST;      //修改状态
	kill(-pid,SIGTSTP);
	printf("Job [%d] (%d) stopped by signal %d\n", pid2jid(pid), pid, sig);
	return;
}
